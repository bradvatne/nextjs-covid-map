...
export default function drawMap(props) {
    ...
    //render function to generate markers with Array.map()
    ...
    const drawMarkers = markersList.map((markerData, index) => 
      ...
      key={index}>
        <Popup>
          <strong>{markerData.countryName}</strong><br/>
          Cases: {markerData.cases}<br/>
          Active: {markerData.active}<br/>
          Deaths: {markerData.deaths}<br/>
        </Popup>
      </Marker>
    )
